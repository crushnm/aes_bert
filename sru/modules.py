...
class SRUCell(nn.Module):
    ...
    def __init__(self,
        ...
        # number of sub-matrices used in SRU 子矩阵数量
        self.num_matrices = 3
...
class SRUppAttention(nn.Module):
    ...
    def __init__(self,
        ...
        # [in_features, proj_features]
        self.linear1 = nn.Linear(in_features, proj_features, bias=False)
        # [proj_features, proj_features * 2]
        self.linear2 = nn.Linear(proj_features, proj_features * 2, bias=False)
        # [proj_features, out_features]
    ...
    def forward(self,
        ...
        src_len = tgt_len = input.size(0)  # 文本长度
        bsz = input.size(1)  # 批处理大小
        in_dim = input.size(2)  # 输入特征维度
        proj_dim = self.proj_features  # 投影维度
        num_heads = self.num_heads  # 头数
        # 投影层维度/头数
        # 1024/8=128
        head_dim = proj_dim // num_heads  # 每个注意力头的维度大小
        scaling = float(head_dim) ** -0.5  # 缩放系数
        ...
        if memory is not None:
            ...
            mem_len = 0
            z = residual = self.linear1(input)  # 残差
            layer_norm = self.layer_norm
            if layer_norm is not None:
                if not self.normalize_after:
                    z = layer_norm(z)  # 归一化
        ...
        # query, key, value
        # self.linear2:[proj_features, proj_features * 2]
        # z:[length, bsz, proj_features]
        # k,v:[length,bsz,proj_features]
        k, v = self.linear2(z).chunk(2, dim=-1)  # 分割的块数 沿着哪个轴分块
        # contiguous():深拷贝
        # view():将原张量元素(按顺序)重组为新的shape -1:自动计算
        # transpose():交换两个维度 (0,1)
        # q:[length, num_heads * bsz, head_dim]->transpose(0,1)
        # q:[num_heads * bsz, length, head_dim]
        ...
        q = q * scaling  # 缩放
        # q:[num_heads * bsz, length, head_dim]
        # k:[num_heads * bsz, head_dim, length]
        attn_output_weights = torch.bmm(q, k.transpose(1, 2))
        # attn_output_weights:[num_heads * bsz,length,length]
        # 维度检查
        ...
        # attn_output:(tgt_len, bsz * num_heads, head_dim)
        # attn_output:(tgt_len, bsz, proj_dim)
...
class SRUppCell(SRUCell):
    ...
    def forward(self,
        ...
        batch_size = input.size(-2)
        if c0 is None:  # c0初始化:[batch_size,output_size]
            c0 = torch.zeros(batch_size, self.output_size, dtype=input.dtype,
                             device=input.device)
        ...
        if self.training and (self.rnn_dropout > 0):
            mask = self.get_dropout_mask_((batch_size, input.size(-1)), self.rnn_dropout)
            input = input * mask.expand_as(input)  # mask某些值
...
class SRUpp(nn.Module):
    ...
    def __init__(self,
                 input_size: int,  # 输入纬度 x:[length, batch_size,input_size]
                 hidden_size: int,  # 新的隐藏维度
                 proj_size: int,  # 投影层维度
                 num_layers: int = 2,  # 模型层数
                 dropout: float = 0.0,  # 随机失活概率
                 attn_dropout: float = 0.0,  # 注意力层的随机失活概率
                 num_heads: int = 1,  # 头数
                 bidirectional: bool = False,  # 是否双向
                 layer_norm: bool = False,  # 是否使用LN
                 normalize_after: bool = True,  # 是否使用后期层规范化
                 highway_bias: float = -2.0,  # 是否使用高速门
                 attention_every_n_layers: int = 1,  # 每几层使用注意力层 默认1->每层都使用
                 rescale: bool = False,  # 是否乘以缩放常量
                 nn_rnn_compatible_return: bool = False,  # 是否共用返回
                 proj_input_to_hidden_first: bool = False,  # 当input_size != hidden_size时 加入nn.Linear()
                 weight_c_init: float = 1.0):  # 均匀初始化
        """Initialize the SRU++ module.
        ...
        self.bidirectional = bidirectional
        self.use_layer_norm = layer_norm
        self.num_directions = 2 if bidirectional else 1  # 方向1、2 由bidirectional决定
        self.nn_rnn_compatible_return = nn_rnn_compatible_return
        self.input_to_hidden: Optional[nn.Module] = None
        # 如果输入维度不等于输出维度 加入线性层 进行维度转换
        if proj_input_to_hidden_first and input_size != self.output_size:
            first_layer_input_size = self.output_size
            # 线性层 [input_size,self.output_size]
            self.input_to_hidden = nn.Linear(input_size, self.output_size, bias=False)
            # xavier_uniform初始化权重矩阵
        ...
        for i in range(num_layers):
            ...
            proj_features = proj_size  # 投影维度大小
            # 输出特征维度
            out_features = self.output_size * (3 if in_features == self.output_size else 4)
            # 得到其中Att层
            custom_m: Optional[nn.Module] = None  # 通用层
            # 每attention_every_n_layers层加入注意力层
            if (i + 1) % attention_every_n_layers == 0:
                ...
                custom_m = SRUppAttention(
                    ...
                    attn_dropout=attn_dropout,  # 注意力随机失活比例
                    num_heads=num_heads,  # 注意力头数
                    layer_norm=layer_norm,  # 层归一化
                    normalize_after=normalize_after,  # 归一化位置
                )
            # 不加入注意力层，则使用线性层替代
            else:
    ...
    def forward(self,
        ...
        length = input.size(0)  # 文本长度
        bsz = input.size(1)  # 批处理大小
        input_size = input.size(2)  # 输入维度
        ...
        i = 0
        x = x.contiguous()  # 深拷贝
        for rnn in self.rnn_lst:  # 循环rnn_list(里面存放的是SRUppCell)
        ...
        # h:(length, batch_size, output_size) output_size = hidden_size * num_direction
        # c:(num_layers, batch_size, output_size ) or (num_layers * num_direction, batch_size, hidden_size)
        # saved_inputs:(num_layers, length, batch_size, output_size)
