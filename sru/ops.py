...
# Skip C++ compilation on Windows if compiler is not available
try:
    ...
    warnings.warn(f"Failed to compile C++ extension: {e}. Falling back to pure Python implementation.")
    USE_CPP = False
...
def elementwise_recurrence_cpu(U: Tensor,
    ...
    """
    if not USE_CPP:
        # Fall back to naive implementation if C++ is not available
        return elementwise_recurrence_naive(U, x, weight_c, bias, c_init,
...
def elementwise_recurrence_naive(U: Tensor,
    ...
    """
    if torch.is_grad_enabled() and USE_CPP:
        warnings.warn("Running SRU on CPU with grad_enabled=True. Are you sure?")
    if USE_CPP and not torch.is_grad_enabled():
